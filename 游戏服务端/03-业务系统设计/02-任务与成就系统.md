...
> 适用范围：覆盖任务/成就在游戏服、事件总线、数据服务与运营配置之间的进度、发奖和审计边界。
> 事实边界：协议规范、组件行为和示例方案分开；容量数字只作示意/经验值，必须通过压测验证。
> 最后更新：2026-08-06（补齐规范来源、版本边界与工程验收入口）。
> 版本边界：以 2026-08-06 为核对日，不新增不确定的当前组件版本号；事件总线、数据库、Redis、协议库和容器版本由项目部署清单锁定。
...
| 验收项 | 设计基线 | 证据入口 |
| --- | --- | --- |
| 业务流水 | 事件、进度变更、完成判定和奖励发放都带 eventId/opId、任务实例、周期和配置版本 | 事件流水与任务状态、奖励流水可逐笔对账，缺口可补偿 |
| 幂等 | 消费者按 eventId，完成/发奖按 questId+cycleId 等业务键去重；重复投递不得重复发奖 | 至少一次投递、消费者重启、超时重试和并发完成测试 |
| 审计 | 保留事件来源、条件命中、配置版本、发奖结果、人工修复和 traceId 关联 | 能定位“为何完成/为何未发奖”，敏感字段脱敏 |
| 服务端权威验证 | 服务端事件和结算结果为主，客户端上报只作受限补充并校验上下文/频率 | 伪造击杀、改进度、改系统时间和断线补发均不能绕过判定 |
| 版本兼容 | 任务实例记录配置版本；进度同步带版本号；新增字段可被旧端忽略 | 热更、回滚、旧客户端、周期切换和迁移均有回放结果 |

> 结论边界：事件最终一致、进度缓存、补偿重试和反刷阈值是方案取舍，不是绝对规则；上线前以配置校验、压测、故障注入和流水对账结果为准。
