...
> 适用范围：覆盖匹配队列、房间生命周期、服务器分配、重连与结算入口，连接网关/登录到房间/游戏服的会话边界。
> 事实边界：协议规范、组件行为和示例方案分开；容量数字只作示意/经验值，必须通过压测验证。
> 最后更新：2026-08-06（补齐规范来源、版本边界与工程验收入口）。
> 版本边界：以 2026-08-06 为核对日，不新增不确定的当前组件版本号；Redis、编排平台、协议库、匹配算法和游戏服版本由项目部署清单锁定。
> 参考来源：[Redis 官方文档](https://redis.io/docs/latest/)、[Kubernetes 官方文档](https://kubernetes.io/docs/)、[Protocol Buffers 官方文档](https://protobuf.dev/)、[OpenTelemetry 官方文档](https://opentelemetry.io/docs/)。

### P0 匹配、房间与会话验收

| 验收项 | 设计基线 | 证据入口 |
| --- | --- | --- |
| 业务流水 | 入队、取消、成组、分配、开局、结算、销毁和重连都记录 ticket/room/session、原因和规则版本 | 队列与房间状态可追溯，结算缺口可补偿且不重复 |
| 幂等 | 入队/取消/确认/创建/销毁按 ticketId、roomId 和状态机条件更新去重 | 重复点击、超时回包、分配重试、服务重启和双重结算均安全 |
| 审计 | 记录匹配规则、候选快照、服务器分配、超时放宽、机器人/惩罚和 traceId 关联 | 客诉可还原“为何匹配/为何失败”，敏感玩家信息脱敏 |
| 服务端权威验证 | 队列成员、队伍权限、房间状态、心跳、准备/开始和结算只认服务端状态 | 改队伍、伪造准备、跳过状态、改延迟或重连 token 均不能越权 |
| 版本兼容 | ticket/房间协议、地图/玩法配置和结算 schema 带版本；新旧网关与游戏服共存有能力协商 | 灰度、回滚、跨版本重连和房间排空均有演练证据 |

> 结论边界：MMR 权重、放宽窗口、超时数值、机器人策略和 HPA/专服扩容都是方案取舍；必须以目标人口、网络分布、压测、故障注入和观测数据验证。
