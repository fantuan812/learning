...
> 适用范围：覆盖网关长连接、聊天服务、社交关系、邮件、公会与运营审计；不把聊天可靠性等同于资产系统可靠性。
> 事实边界：协议规范、组件行为和示例方案分开；容量数字只作示意/经验值，必须通过压测验证。
> 最后更新：2026-08-06（补齐规范来源、版本边界与工程验收入口）。
> 版本边界：以 2026-08-06 为核对日，不新增不确定的当前组件版本号；WebSocket/协议库、Redis、数据库、过滤服务和容器版本由项目部署清单锁定。
> 参考来源：[WebSocket RFC 6455](https://www.rfc-editor.org/rfc/rfc6455.html)、[Redis 官方文档](https://redis.io/docs/latest/)、[Protocol Buffers 官方文档](https://protobuf.dev/)、[OpenTelemetry 官方文档](https://opentelemetry.io/docs/)。

### P0 消息、关系与审计验收

| 验收项 | 设计基线 | 证据入口 |
| --- | --- | --- |
| 业务流水 | 消息、邮件投递/领取、好友/公会关系变化带 messageId/mailId、操作者、原因和规则版本 | 离线补发、附件领取、关系变更和举报记录可对账 |
| 幂等 | 消息发送/消费、邮件领取和公会操作按业务键去重；聊天可丢弃不等于资产附件可重复领取 | 重连、重复点击、跨服重试和消费者重启不重复发附件或变更关系 |
| 审计 | 按政策留存内容摘要/引用、频道、发送者、处置、申诉和 traceId 关联；敏感内容按权限访问 | 客服/合规可查询，删除/封禁/解封有审批与证据链 |
| 服务端权威验证 | 服务端校验频道成员、黑名单、禁言、权限、附件和关系状态，客户端只提交意图 | 改频道、伪造管理员、绕过敏感词/限流和篡改附件均被拒绝 |
| 版本兼容 | WebSocket/protobuf 消息只增不复用字段号，频道/关系状态带版本；旧客户端可降级 | 新旧客户端、网关滚动发布、重连补历史和协议回滚均可验证 |

> 结论边界：聊天丢弃、历史保留、过滤阈值、Redis 缓存和跨服广播方式都是方案取舍；上线前用真实长连接、广播扇出、恶意刷屏、断连和审计查询压测验证。
