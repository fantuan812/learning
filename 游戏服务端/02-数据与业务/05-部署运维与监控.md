...
- 开服/合服是运维操作，顺序启停可控；
- HPA 自动扩缩对游戏服意义不大——玩家不能凭空迁移，扩容通常以"新开一个服"为单位（手动/编排平台操作）。
...
OpenTelemetry（OTel）标准：Trace（一次请求的完整链路）→ Span（链路中的一段调用）→ 上下文传播（trace_id/span_id 透传）。
...
2. **编排**：无状态服务 Deployment + HPA；游戏服 StatefulSet + 手动开服；数据库/缓存独立部署（勿容器化生产库）；
...
**Q2：HPA 能用于游戏服吗？**
A：不能直接用于"玩家数据在服内"的游戏服——Pod 数变了玩家连不上。正确做法：①无状态子系统（网关/登录）用 HPA；②游戏服以"服"为单位扩容（新开 gs-4），由运营平台编排（创建 StatefulSet 副本 + 注册中心注册 + 开服公告）。
