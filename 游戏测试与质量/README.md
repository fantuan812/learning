...
| [07-UE DS机器人压测与容量评估.md](<07-UE DS机器人压测与容量评估.md>) | UE5.8 Dedicated Server 机器人负载形态、场景混合、阶梯加压、容量拐点、服务端/客户端指标与单实例/区域容量评估模型 |
...
**阶段五：Dedicated Server 容量评估**

7. 阅读 [07-UE DS机器人压测与容量评估](<07-UE DS机器人压测与容量评估.md>)。前置知识：`06`（联机验收）与 [游戏知识](../游戏知识/README.md) 的 `08-工具链与打包发布/10-UE Dedicated Server运行参数与性能调优`。目标是能用真实协议机器人驱动 DS、识别容量拐点并产出容量评估报告。

**阶段六：非功能验证**

8. 阅读 [04-性能兼容与网络异常测试](04-性能兼容与网络异常测试.md)。前置知识：[游戏知识](../游戏知识/README.md) 的 `07-UI与性能优化`、`06-网络同步`。建议配合一台真机与一台低端机实操。

**阶段七：工程化收口**

9. 阅读 [05-CI质量门禁与缺陷管理](05-CI质量门禁与缺陷管理.md)。前置知识：`02`、`03`、`04`、`06`、`07` 的可执行产物（测试命令、报告格式）。目标是能把"质量红线"写进流水线，把"缺陷"管成闭环。
...
| 想知道一台 DS 能扛多少玩家、拐点在哪？ | [07（机器人压测与容量评估）](<07-UE DS机器人压测与容量评估.md>) + 游戏知识 08（运行参数与性能调优） |
...
- Dedicated Server 相关新增：联机验收与 Gauntlet 归入 06，机器人压测与容量评估归入 07；
...
└── 07-UE DS机器人压测与容量评估.md  # 机器人负载 / 阶梯加压 / 容量拐点 / 容量评估
```
