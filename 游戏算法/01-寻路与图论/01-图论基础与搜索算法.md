...
> 适用范围：离散网格、导航图与一般加权图；示例可用于单机或服务端计算，静态地图可离线预处理，运行时搜索也适用；不覆盖连续空间中的精确几何导航。
> 数值与正确性边界：BFS 仅在边权相同前提下给出最短路；Dijkstra 要求边权非负；复杂度按顶点数、边数和邻居生成方式计，浮点代价的比较需约定容差或稳定排序，经验耗时必须实测。
> 最后更新：2026-08-06（补齐来源、边界条件与验证入口）。
> 参考来源：[Red Blob Games：A* 与图搜索可视化](https://www.redblobgames.com/pathfinding/a-star/)。
...
- 输入规模与边界用例：覆盖空图、单点、重复边、环、不可达顶点、孤立连通分量和大规模稀疏图；加权测试分别覆盖全等权、非负整数权和非法负权输入。
- 正确性断言：BFS 在等权图上的路径边数应等于最短边数；Dijkstra 的 `dist` 应满足每条边的松弛不等式，能到达的终点回溯后首尾正确且每条边都存在。
- 复杂度与耗时指标：记录 `V`、`E`、访问顶点数、最大队列/堆大小和端到端耗时；分别观察邻接矩阵的 `O(V²)` 与邻接表/堆实现的 `O(V+E)` 或 `O((V+E)logV)`，不把理论复杂度当作实测结论。
- 命令示意（仅示意，不表示仓库已有测试）：`python -m pytest tests/test_graph_search.py -q`；C++ 项目可替换为自身的单元测试或 benchmark 目标。
...
5. **游戏帧预算**：部分大地图单次搜索可能超过 1ms，这只是风险提示，是否超出预算以目标平台实测为准；把搜索拆到多帧（时间片轮转）或预计算（离线烘焙）；
