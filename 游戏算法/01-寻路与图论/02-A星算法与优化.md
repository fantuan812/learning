...
从 01 篇我们知道：Dijkstra 能找到加权图的最短路径，但它"往所有方向均匀扩张"，在"起点→单个目标"的寻路场景里做了大量无用功。A*（A-Star，1968 年由 Hart、Nilsson、Raphael 提出）在 Dijkstra 的基础上加入**启发函数（Heuristic）**，让搜索"偏向目标方向"扩张，在**保证最优性**的前提下大幅减少搜索量。

A* 是游戏寻路的绝对主流：

- Unity 的 NavMesh 寻路（NavMeshAgent）内部基于 A* 及其变体；
- Unreal Engine 的 Navigation System 同样以 A* 为核心；
...
| 可采纳性 | Admissible | 对任意 n：h(n) ≤ h*(n)（h* 为真实最短代价） | 可采纳 ⇒ A* 找到的路径是最优的 |
| 一致性 | Consistent | 对任意边 (n, n')：h(n) ≤ w(n,n') + h(n') | 一致性 ⇒ 可采纳，且每个顶点最多扩展一次 |
| 最优性 | Optimality | 算法返回的路径代价等于真实最小代价 | A* 在可采纳启发下具备该性质 |
...
| 启发函数 | 公式 | 适用移动方式 | 特点 |
| --- | --- | --- | --- |
| 曼哈顿距离 | h = \|dx\| + \|dy\| | 4 方向移动 | 对 8 方向移动会**低估**真实代价（不精确但可采纳），搜索略慢但保证最优 |
| 欧几里得距离 | h = √(dx² + dy²) | 任意角度移动（连续世界） | 对 8 方向移动略微低估，可采纳 |
| 切比雪夫距离 | h = max(\|dx\|, \|dy\|) | 8 方向、步长一致 | 对 8 方向移动**恰好等于**真实代价（当无地形差异时），搜索最快且最优 |
| 对角线距离 | h = (dx + dy) + (√2 − 2)·min(dx, dy) | 8 方向、斜向代价 √2 | 精确匹配网格真实代价，工程首选 |

> **关键原则**：h 必须与**实际移动方式**匹配。若角色只能 4 方向移动却用切比雪夫启发（高估真实代价），A* 可能返回非最优路径（见第 5 节）。斜向移动代价是 √2 ≈ 1.414 而非 1，忽略这点会导致启发高估。
...
**定理：若 h 可采纳，则 A* 找到的路径是最优的。**

证明思路（反证）：

1. 假设 A* 返回路径 P 的代价 C > C*（真实最优代价）；
2. A* 结束时目标 G 被扩展，说明扩展 G 时 `f(G) = g(G) = C`；
3. 在真实最优路径 P* 上，必存在一个顶点 n 尚未被扩展（否则 P* 已被还原）。对最优路径上的任意顶点 n，因为 h 可采纳：
   `f(n) = g(n) + h(n) ≤ g*(n) + h*(n) = C*`
   （g(n) ≤ g*(n)，因为 g 是某条可行路径前缀的实际代价，不可能大于最优前缀代价）
...
3. 所有常用启发（曼哈顿、欧几里得、切比雪夫、对角线）在正权图中都满足一致性。
...
> 工业界标杆是 Recast/Detour（开源库）：Recast 负责从几何体烘焙 NavMesh，Detour 提供 A* 寻路与漏斗平滑。Unity、Unreal 的导航系统都受其影响。
...
4. **帧预算与时间片**：超时（如 2 ms）则返回"当前最优前缀路径"或延迟到下一帧继续（迭代加深 A* / IDA* 是另一条路）；
...
**Q1：A* 一定返回最优路径吗？**
当且仅当启发函数可采纳（不高估）时保证最优。使用权重化 A*（w > 1）或过高估计的启发（如 8 方向用曼哈顿×√2）都会破坏最优性。

**Q2：为什么 8 方向移动不能用曼哈顿启发？**
可以用但效率低：曼哈顿低估真实代价（真实斜走 1.41 < 曼哈顿 2），搜索会扩展更多顶点；但不会破坏最优性。反过来，4 方向移动用切比雪夫启发会**高估**，破坏最优性——启发必须匹配移动方式。
...
能，这正是 NavMesh 寻路的标准做法：多边形图 + A*（欧几里得启发）+ 漏斗平滑，参考 Recast/Detour。
