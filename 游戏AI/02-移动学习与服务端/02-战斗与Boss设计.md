...
- 固定输入/场景：固定竞技场/导航数据、Boss 初始生命值与阶段阈值、玩家初始站位、伤害/打断事件序列、随机种子和 `dt`，回放一场包含目标选择、施法前摇、阶段切换、脱战和胜利清理的完整遭遇。
- 状态或行为正确性断言：同一回放的目标与阶段轨迹应稳定；技能只有在前置条件、冷却、距离/视线规则满足时才生效，预警先于效果，阶段切换符合单调或显式回退规则，Boss 死亡/脱战后不再产生决策或技能副作用。
- 指标：记录目标切换次数、阶段/状态切换次数、决策延迟 P50/P95/P99、技能选择成功率、技能命中/打断/取消率、预警提前时间、施法排队等待、召唤单位数量和遭遇时长分布；不填入未经实测的阈值或性能数字。
- 边界用例：覆盖无目标/目标全死亡、威胁值并列、所有技能都在冷却、生命值恰好命中阶段阈值、前摇开始或结束时被打断、目标在范围技能前摇期间离开、狂暴计时到期、脱战后重新进入，以及死亡清理未完成。
- 命令示意（仅示意，不表示仓库已有测试）：`python -m pytest tests/test_boss_ai.py -q`；C++ 可接入项目自有遭遇回放/统计命令，按阶段输出技能成功率、预警时序和决策耗时。
