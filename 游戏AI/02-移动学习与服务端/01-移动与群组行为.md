...
> 知识基线：引擎无关的游戏 AI 通用原理；涉及 UE 的实现以[游戏知识/05-AI系统/03-NavMesh寻路.md](../../游戏知识/05-AI系统/03-NavMesh寻路.md)和[游戏知识/05-AI系统/04-Mass实体框架与群集模拟.md](../../游戏知识/05-AI系统/04-Mass实体框架与群集模拟.md)为交叉验证边界。
> 适用范围：适用于单机/客户端角色移动与群组行为、服务端权威移动和大规模群体模拟、AI 策划队形与避障参数设计，以及跨引擎的寻路—Steering—表现层协作；网络同步和目标平台性能需单独验收。
> 事实边界：算法/架构结论与具体引擎、模型、平台版本分开；Steering、Boids、流场、RVO/ORCA 是通用算法或案例经验，库与引擎实现、代理规模和参数效果需按项目评估；代码片段为示意。
> 最后更新：2026-08-06（补齐来源、适用范围与事实边界）。
> 参考来源：[Craig Reynolds 的 Boids 资料](https://www.red3d.com/cwr/boids/)；[Unreal Engine 官方文档总页](https://dev.epicgames.com/documentation/en-us/unreal-engine)；交叉阅读：[游戏知识/05-AI系统/03-NavMesh寻路.md](../../游戏知识/05-AI系统/03-NavMesh寻路.md)、[游戏知识/05-AI系统/04-Mass实体框架与群集模拟.md](../../游戏知识/05-AI系统/04-Mass实体框架与群集模拟.md)。
...
群集产生"自然的乱"，但玩家期望军队走**整齐的队形**。两种常见做法：
...
> 边界说明：RVO/ORCA 在本文中是局部避障的算法案例与工程经验；不同库、引擎和网络权威模型的行为与规模上限不能直接互换，需按速度模型、代理半径、拥挤度和性能预算评估；上面的实现是概念级示意。
...
- 固定输入/场景：固定地图/导航数据、代理半径、初始位置与速度、目标点、编队槽位、邻居快照、`dt` 和随机种子，准备单体到达、双向相遇、窄门编队、动态障碍和目标不可达等可重复场景。
- 状态或行为正确性断言：可达目标应到达或在超时后明确报告失败，不可达目标应触发约定的重规划/兜底；单位不得穿透碰撞体，速度/加速度满足配置约束，编队槽位误差在项目容差内，固定输入回放的输出满足确定性要求。
- 指标：记录单位数量、到达率与到达时间、路径长度/偏差、最小间距、碰撞率、卡死与重规划次数、邻居候选数、编队槽位误差、决策/Steering 延迟 P50/P95/P99；将全局寻路、局部避障和表现更新分段统计。
- 边界用例：覆盖零单位/空邻居、起点等于终点、零速度、窄通道或无可行路径、动态障碍突然出现/消失、最大编队规模、目标在更新期间移动，以及数值出现 NaN/Inf 的输入。
- 命令示意（仅示意，不表示仓库已有测试）：`python -m pytest tests/test_movement_group.py -q`；C++ 可接入项目自有回放或 benchmark，对寻路、编队、Steering 与 RVO/ORCA 分段统计。
