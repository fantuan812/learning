...
> 源码依据：本机只读安装目录 `C:\Program Files\Epic Games\UE_5.8\Engine`，重点覆盖 `Runtime/Core` 的 Containers、Templates、HAL 与 Memory 实现。
> 适用范围：桌面、主机和移动目标上的 Core 容器、智能指针与统一内存入口；编辑器、运行时、客户端和服务端都适用，但线程安全与 UObject GC 规则必须分别判断。
> 兼容性边界：UE 4.27 及 UE 5.0–5.7 仅用于迁移对照；容器布局、分配器实现和私有符号不能视为跨版本 ABI，当前结论以本机 UE 5.8 为准。
> 官方参考：[Unreal Engine 官方文档总页](https://dev.epicgames.com/documentation/en-us/unreal-engine)。
> 最后更新：2026-08-06（补齐 UE5.8 元数据、官方入口与容器源码验收边界）。
...
| 核心头文件 | Array.h、Map.h、Set.h、SparseArray.h、SharedPointer.h、UnrealMemory.h、MallocBinned.h（均位于 Public/ 下；MallocBinned2.h/3.h 也在 Public/HAL/） |
...
| --- | --- |
| `Public/Containers/Array.h` | TArray 声明与实现：分配器增长（CalculateSlackGrow）、RelocateConstructItems 元素管理（5.8：FScriptArray 已移至 ScriptArray.h，ReserveGrow 不存在） |
| `Public/Containers/ContainerAllocationPolicies.h` | 分配器抽象：FDefaultAllocator、FHeapAllocator、TSizedHeapAllocator、CalculateSlackGrow/DefaultCalculateSlackGrow 系列（5.8 由 ArrayAllocator.h 更名） |
| `Public/Containers/Map.h`（实现见 `Map.h.inl`） | TMap/TMapBase：TPair 元素集、Find/Emplace、ByHash 系列 |
| `Public/Containers/Set.h`（实现见 `CompactSet.h`/`SparseSet.h`） | TSet：5.8 由宏注入定义为紧凑集（默认 TSet=TCompactSet 布局） |
| `Public/Containers/SparseArray.h` | TSparseArray：稀疏数组、空闲索引列表、迭代器 |
| `Public/Containers/HashTable.h` | THashTable：哈希桶（桶头数组 + 冲突链） |
| `Public/Templates/SharedPointer.h` | TSharedPtr/TSharedRef/TWeakPtr 公开接口（5.8 位于 Templates/） |
| `Public/Templates/SharedPointerInternals.h` | FSharedReferencer、FWeakReferencer、TReferenceControllerBase、原子引用计数实现（FSharedPtrState 已移除） |
| `Public/HAL/UnrealMemory.h` | FMemory 接口：Malloc/Free/Realloc（5.8：GetAllocator 已移除，全局分配器为 UE::Private::GMalloc） |
| `Private/HAL/UnrealMemory.cpp` | FMemory 实现：平台内存、对齐、统计 |
| `Private/HAL/MallocBinned.cpp` | FMallocBinned：第一代分桶分配器 |
| `Private/HAL/MallocBinned2.cpp`（头文件 `Public/HAL/MallocBinned2.h`） | FMallocBinned2：改进版（无锁路径、缓存行友好） |
| `Private/HAL/MallocBinned3.cpp`（头文件 `Public/HAL/MallocBinned3.h`） | FMallocBinned3：UE5 新一代（64KB 大页、多级池） |

### 2.1 源码与运行时验证命令

以下命令只读访问 UE5.8 安装目录，用于重新确认本文的路径和代表符号；`rg` 的命中行号会随引擎小版本变化，不能把行号当作 ABI。

```powershell
$ue = 'C:\Program Files\Epic Games\UE_5.8\Engine'
Test-Path "$ue\Build\Build.version"
Test-Path "$ue\Source\Runtime\Core\Public\Containers\Array.h"
Test-Path "$ue\Source\Runtime\Core\Public\Templates\SharedPointerInternals.h"
rg -n 'CalculateSlackGrow|RelocateConstructItems|class TMapBase|class TReferenceControllerBase' "$ue\Source\Runtime\Core"
rg -n 'FMemory::Malloc|GMalloc|FMallocBinned2|FMallocBinned3' "$ue\Source\Runtime\Core"
```

运行时可在 Development 配置使用 `stat memory`、`memreport` 对照容器复用、池容量和 UObject/GC 统计；详细分配归因再用 Unreal Insights 的 Memory 轨迹验证，不要只根据单次总量快照判断泄漏。

## 三、TArray：连续内存的动态数组

### 3.1 内存模型：FScriptArray

`TArray<ElementType>` 的本质是一个 `FScriptArray`（ScriptArray.h，5.8）：

```cpp
class FScriptArray : public TScriptArray<FHeapAllocator>
{
public:
	// 无类型数据数组：Data/ArrayNum/ArrayMax 语义不变，
	// 数据指针与分配器实例由基类 FHeapAllocator::ForAnyElementType 持有（5.8）
	void* GetData();        // 元素数据起始地址（堆内存）
	int32 ArrayNum;         // 当前元素个数（逻辑大小）
	int32 ArrayMax;         // 已分配容量（可容纳的最大元素数，>= ArrayNum）
	// ...
};
```
...
`ArrayMax > ArrayNum` 的余量就是**扩容余量**：`Add` 元素时只要 `ArrayNum < ArrayMax`，直接写入内存 + `ArrayNum++`，**零分配**。只有 `ArrayNum == ArrayMax` 时才触发扩容（见 3.2）。

### 3.2 扩容策略：CalculateSlackGrow 与 ReallocGrow（5.8）

```cpp
// TArray 内部（Array.h，5.8）：空间不足时增长
// 5.8 中已无 ResizeGrow 成员，扩容统一走分配器：
NewMax = AllocatorInstance.CalculateSlackGrow(NewNum, OldMax, ElementSize, ElementAlignment);
// + ReallocGrow1_DoAlloc 系列重新分配 Data 并搬移旧元素（约 1.5 倍增长，首次至少 8 个元素）

// FDefaultAllocator（ContainerAllocationPolicies.h，5.8 由 ArrayAllocator.h 更名）
static int32 CalculateSlackGrow(SizeType NewMax, SizeType CurrentMax, SIZE_T NumBytesPerElement);
// 增长/收缩/显式容量申请分别对应 DefaultCalculateSlackGrow / DefaultCalculateSlackShrink / DefaultCalculateSlackReserve
```

要点：

1. **1.5 倍增长**：避免"每次 +1"导致反复 realloc 的平方级复杂度；1.5 倍是时间（复制次数）与空间（浪费率 ~33%）的经典折中。`Add` N 个元素的总摊还成本 O(N)。
2. `DefaultCalculateSlackGrow`（5.8） 还会做**对齐与最小容量**处理（至少容纳当前元素、按分配器粒度取整），并把"是否允许收缩（bAllowShrinking）"考虑进去。
...
> 原稿引用的 `TArray::CopyForShipping` 在 UE 5.8 源码中不存在（也不是 TArray 的公开 API）。引擎真正用于“把数组冻结为只读打包镜像”的机制是 `Core/Public/Serialization/MemoryImage.h` 的 `FMemoryImageWriter` 等：Shipping 构建下目标数组的 `Data` 可以直接指向打包镜像、元素不执行构造函数（镜像里已是最终内存布局）。“零成本加载”的结论仍成立，但请以上述真实 API 为准。
...
class TMapBase
{
	// TMap 内部就是一个"以 TPair 为元素类型"的 TSet 变体（5.8 定义在 Map.h.inl）
	using ElementSetType = TSet<TPair<KeyType, ValueType>, KeyFuncs, SetAllocator>;
	// ...
};

// TSet 的定义（5.8）：Set.h 通过宏注入定义 TSet，默认 TSet=TCompactSet（紧凑布局），另有 TSparseSet；
	// 下图为经典的稀疏布局原理（TSparseArray + THashTable），适用于 TSparseSet 与 UE4 旧版 TSet
...
TPair<KeyInitType, ValueInitType>* TMapBase<...>::Find(const KeyType& Key)
{
	...
	// 2. 沿桶内冲突链逐个比对 Key
	if (auto* Pair = Pairs.FindByHash(KeyHash, Key))   // 5.8：TConstSetIterator 已移除
	{
		if (Pair->Key == Key)   // KeyFuncs::Matches 判定相等
		{
			return const_cast<TPair<KeyInitType, ValueInitType>*>(Pair);
		}
	}
...
5.8 中链走前进封装在紧凑哈希实现内部（TConstSetIterator 与 Hash.Next 不再是公开接口）。整体成本：哈希计算 O(1) + 链长（负载因子低时≈1）。
...
TPair<KeyInitType, ValueInitType>& Emplace(KeyInitType&& InKey, ValueArgTypes&&... InValueArgs)
{
	...
	if (Elements.Num() > Hash.Num() * HashSizeScale)   // 5.8：HashSizeScale 已移除，扩容阈值在紧凑哈希实现内部管理
	{
		Hash.Resize(Elements.Num() * HashSizeScale);   // 5.8：HashSizeScale 已移除，扩容阈值在紧凑哈希实现内部管理
	}
...
// 更精细的控制：自定义 KeyFuncs（Map.h 中 TDefaultMapHashableKeyFuncs 的模式，5.8 位于 Map.h）
...
```cpp
// SharedPointerInternals.h（5.8：FSharedPtrState 已移除，状态块改为 TReferenceControllerBase）
template <ESPMode Mode>
class TReferenceControllerBase
{
	RefCountType SharedReferenceCount{1};   // 强引用计数
	RefCountType WeakReferenceCount{1};     // 弱引用计数
	virtual void DestroyObject() = 0;       // 强引用归零时销毁真实对象
};

class FSharedReferencer
{
	TReferenceControllerBase<Mode>* ReferenceController;   // 指向共享状态块
	void AddSharedReference()     { /* 原子递增 SharedReferenceCount（语义等价 InterlockedIncrement） */ }
	void ReleaseSharedReference() { /* 原子递减；归零 → DestroyObject()（弱引用状态保留） */ }
	// ...
};
```
...
```text
                    TReferenceControllerBase（堆上共享块）
   TSharedPtr A  ──►  ┌────────────────────────┐
...
```cpp
enum class ESPMode : uint8   // 5.8：定义在 Templates/SharedPointerFwd.h（SharedPointer.h 引用）
{
...
struct FMemory
{
	...
	// GetAllocator() 已移除（5.8）；全局分配器为 UE::Private::GMalloc（MemoryBase.h 保留未命名空间兼容别名）
	// ...
};
```

`UE::Private::GMalloc` 是全局分配器指针（5.8 名称空间化），在引擎启动早期由 `FPlatformMemory::BaseAllocator()` 创建（UnrealMemory.cpp），并随平台/配置切换：Debug 用 `FMallocAnsi`（直接 CRT malloc，便于 Valgrind/ASan）、正式构建用 `FMallocBinned2`/`FMallocBinned3`。所有容器（TArray/TMap/FString）与 `new`（UObject 走 `FMemory::Malloc` 的全局 operator new 重载）都汇入这条管线——这是引擎能做内存统计（`stat memory`）与内存泄漏追踪的前提。
...
```cpp
// MallocBinned2.h（5.8：BINNED2_MINIMUM_ALIGNMENT 已移除，常量改为 UE_MB2_* 前缀）
#define UE_MB2_LARGE_ALLOC 65536   // 大块分配阈值（OS 分配对齐）
// 对齐规则见 HAL/MemoryBase.h：DEFAULT_ALIGNMENT=0（默认，≥16B 按 16B、<16B 按 8B）、MIN_ALIGNMENT=8
```

| 分配器 | 版本 | 核心机制 | 特点 |
| --- | --- | --- | --- |
| `FMallocBinned` | UE4 早期 | 固定大小桶（16B 步进）+ 池 | 简单，桶多，内部碎片一般 |
| `FMallocBinned2` | UE4 中期（UE4 默认） | SizeClass 优化（32 个类）、无锁路径、缓存行对齐 | 高并发分配性能好 |
| `FMallocBinned3` | UE5（5.8 中 64 位平台默认） | 64KB 大页 + Chunk 分级池 + 位图 | 进一步降低碎片与 TLB 压力 |
...
- `DEFAULT_ALIGNMENT = 0`（5.8）：表示“引擎默认对齐”——≥16 字节的块按 16 字节对齐，<16 字节按 8 字节（MIN_ALIGNMENT），满足 SSE/NEON 向量类型需求。
...
sequenceDiagram
    ...
    participant State as TReferenceControllerBase
...
① 每个分配有头部元数据（桶索引、大小）；② 桶粒度导致内部碎片（申请 17B 实际占 32B 桶）；③ 池预分配（大块 Pool 按页切分，整页预取）；④ 对齐 padding。`stat memory` 里的 "OS Alloc" 与 "Pool" 差值反映池容量与实际使用量的差异。小对象合并（如 `TArray` 存值而非指针）是减少这类开销的直接手段。
...
- [07-UI与性能优化/04-渲染与加载性能优化](../07-UI与性能优化/04-渲染与加载性能优化.md)（打包镜像与加载优化思路）
