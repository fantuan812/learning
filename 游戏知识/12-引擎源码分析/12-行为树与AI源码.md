...
| AIModule | `Classes/BehaviorTree/BlackboardComponent.h` + `Private/BehaviorTree/BlackboardComponent.cpp` | `UBlackboardComponent`、`SetValueAsBool / SetValueAsVector`、`GetValueAsBool / GetValueAsVector`、`RegisterObserver`、`GetKeyID` | 黑板数据与观察者 |
...
版本注：`ExecuteTree(UBehaviorTree& InAsset, UBTNode* InRootOverride)` 并非 UE 官方 API（5.8 的 AIModule 源码中不存在该方法，部分旧教程的误称）；行为树启动入口自 UE4 起始终是 `StartTree(UBehaviorTree& Asset, EBTExecutionMode::Type ExecuteMode)`。`EBTExecutionMode` 只有 `SingleRun` 与 `Looped` 两值（`BehaviorTreeTypes.h`）：
...
- `InstanceMemory` 是一整块连续内存，按编译期算好的偏移（`GetInstanceMemorySize()` / `InitializeNode` 里的 `InMemoryOffset`）切给每个节点；
...
节点模板属性在 `UBehaviorTree::RootNode` 下组织；`UBehaviorTreeComponent` 持有一个实例栈 `TArray<FBehaviorTreeInstance> InstanceStack`（子树通过 `PushInstance(UBehaviorTree& TreeAsset)` 压栈嵌套，如 BTTask_RunBehavior），以及 `KnownInstances`（实例标识符列表，用于调试器回溯）。
...
- `WrappedTickNode` 内部先检查"距上次 Tick 是否已过 Interval"（5.8 中 tick 时间直接写入节点内存，由 `UBTAuxiliaryNode::SetNextTickTime / GetNextTickRemainingTime` 读写，无独立的 `FBTServiceMemory` 结构），未到期直接返回 false，从而支持"比帧率更低频"的服务；
...
服务的典型实现（如 `UBTService_BlackboardBase` 系：`UBTService_DefaultFocus`、`UBTService_RunEQS`；`UBTService_UpdateBoundingBox` 已在 5.8 移除）就是在 `TickNode` 里写黑板键。
...
| 蓝图节点（BlackboardBase 系） | `GetBlackboardValueAsVector` 等 | 本质是 `UBlackboardComponent::GetValueAsVector / GetValueAsBool` 的封装 |
...
- `stat AI` / `stat BehaviorTree`：`STAT_AI_BehaviorTree_Tick`、`STAT_AI_BehaviorTree_SearchTime`、`STAT_AI_BehaviorTree_StopTree` 等计数器直接对应本篇剖析的函数；
...
### Q7：`StartTree` 与网上资料常说的 `ExecuteTree` 是什么关系？

`ExecuteTree(UBehaviorTree& InAsset, UBTNode* InRootOverride)` 不是 UE 官方 API：5.8 的 AIModule 源码中不存在该方法，自 UE4 起行为树启动入口一直是 `StartTree(UBehaviorTree& Asset, EBTExecutionMode::Type ExecuteMode)`（5.8 中重启用 `RestartTree(EBTRestartMode RestartMode = EBTRestartMode::ForceReevaluateRootNode)`）。看到旧教程写 `ExecuteTree` 时，直接按 `StartTree` 理解即可。
