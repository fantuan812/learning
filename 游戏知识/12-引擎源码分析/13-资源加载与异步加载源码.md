...
| CoreUObject | `CoreUObject/Private/Serialization/AsyncLoading.cpp` | `FAsyncPackage`（UE4.27~5.0 旧系统）、`FAsyncLoadingThread` | 旧版异步加载（版本对比用） |
...
```cpp
UObject* StaticLoadObjectInternal(UClass* ObjectClass, UObject* InOuter, FStringView InName, FStringView Filename, uint32 LoadFlags, UPackageMap* Sandbox, const FLinkerInstancingContext* InstancingContext)
{
	// ...
...
> 注意 `ResolveName` 在 5.x 的头文件里仍是公开声明（`COREUOBJECT_API bool ResolveName(UObject*& Outer, FString& ObjectsReferenceString, bool Create, bool Throw, uint32 LoadFlags = LOAD_None);`，UObjectGlobals.h），但路径解析内部已经大幅迁移到 `FPackagePath` / `FTopLevelAssetPath` 体系（见 3.2）。
...
这是 UE 5.x 最重要的架构事实：**同步加载在引擎内部被实现为"异步请求 + 针对该 RequestID 的 FlushAsyncLoading"**。`LoadPackage(UPackage* InOuter, const TCHAR* InLongPackageNameOrFilename, uint32 LoadFlags, ...)` 只是 `LoadPackageInternal` 的一层壳（负责把字符串转成 `FPackagePath`）。
...
```cpp
	...
	TSharedPtr<FStreamableHandle> RequestAsyncLoad(
		...
		FStreamableDelegateWithHandle DelegateToCall = FStreamableDelegateWithHandle(),
	...
	ENGINE_API TSharedPtr<FStreamableHandle> RequestSyncLoad(
		TArray<FSoftObjectPath> TargetsToStream,
		bool bManageActiveHandle = true,
		FString DebugName = TEXT("RequestSyncLoad")
	);
...
- `RequestAsyncLoad` 把请求打包进 `FStreamableAsyncLoadParams`（内含 `OnComplete` / `OnCancel` 委托、优先级、`DebugName`），交给内部 `RequestAsyncLoadInternal`；
- `RequestAsyncLoadInternal` 会先做"去重合并"（同路径已有请求则挂到同一句柄上，避免重复加载同一资源），再对每个目标调用 `LoadPackageAsync`（底层），并登记"该目标完成时 → 推进句柄计数"；
- 句柄计数归零（所有目标加载完成）时，`FStreamableHandle` 在游戏线程触发 `CompleteDelegate`——**回调一定在游戏线程**（`FStreamableManager::ProcessAsyncLoadRequests` 由引擎每帧驱动）；
- `RequestSyncLoad` 内部就是 `RequestAsyncLoadInternal` + `WaitUntilComplete()`（`FStreamableHandle::WaitUntilComplete(float Timeout, bool bStartStalledHandles)`），与 3.2 节"同步 = 异步 + 阻塞"的结论一致。
...
COREUOBJECT_API int32 LoadPackageAsync(const FString& InName, FLoadPackageAsyncDelegate InCompletionDelegate, TAsyncLoadPriority InPackagePriority = DefaultAsyncLoadPriority, EPackageFlags InPackageFilter = PKG_None, FLinkerInstancingContext* InstancingContext = nullptr);
```
...
- **加载线程池**（`FAsyncLoadingThread2`）：`StartLoading` 起的 IO 请求交给 IoStore/文件系统，反序列化与依赖推进在工作线程执行；每个包内部状态机（`EAsyncPackageLoadingState2`：`WaitingForIO / LinkerCreated / ExportsDone / PostLoad / DeferredPostLoad / Complete` 等）由事件驱动流转；
...
> 版本注：UE 4.27 ~ 5.0 的旧系统是 `FAsyncPackage`（AsyncLoading.cpp），包内为显式状态机（`FAsyncPackage::Tick` 推进 `SetupDependencies → CreateLinker → LoadAllObjects → PostLoadObjects → CompletionCallback`）。5.1 起迁移到 EDL2（`FAsyncPackage2`），行为语义一致，但"依赖就绪"由事件图驱动、并行度更高。看到网上老文章讲 `FAsyncPackage::Tick` 时，注意对应旧版本。
...
COREUOBJECT_API EAsyncPackageState::Type ProcessAsyncLoadingUntilComplete(TFunctionRef<bool()> CompletionPredicate, float TimeLimit = 0.0f);
```
...
不会。`RequestAsyncLoadInternal` 有请求合并：同一 `FSoftObjectPath` 在途时，新请求挂到已有加载上；两个句柄都会在完成时收到回调。这正是"多个 UI 同时要同一个头像"场景不重复加载的原因。
