...
| Chaos | `Experimental/Chaos/Public/Chaos/Framework/Threading.h` | `FPhysicsThreadContext`（L171）、`CHAOS_SCENE_LOCK_TYPE` | 线程上下文（非 Shipping 编译）与场景锁类型 |
...
```cpp
// Engine/Source/Runtime/Experimental/Chaos/Public/Chaos/Framework/Threading.h（节选，5.8）
// Scene lock types
...
- `Advance` / `AdvanceOneTimeStep` 是物理线程每步执行的函数：重力 → 外力 → 积分 →
  ...
  `Chaos/Private/PBDRigidsEvolutionGBF.cpp`）。
...
- 一个 `FPBDCollisionConstraint` 描述**一对形状之间的一次接触**（接触流形），
  由窄相检测（`CollisionDetection.cpp`）生成后放进 `FPBDCollisionConstraints` 容器。
...
| `FPhysicsThreadContext` | `Chaos/Public/Chaos/Framework/Threading.h` | 线程语义上下文（调试用，非 Shipping 编译） |
