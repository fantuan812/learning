...
- 蒙太奇的 Root Motion 配置（`bExtractRootMotionFromAnim` 等）决定播放时角色是否被动画带动位移，详见 03 篇。
...
> 注：UE5 的 Notify 回调签名带 `const FAnimNotifyEventReference&` 参数；UE 4.27 及更早版本无此参数，移植时注意。
...
多数是 Root Motion 问题：蒙太奇勾选了提取 Root Motion 而移动组件没有消费，或反之。检查蒙太奇资产的 `bExtractRootMotionFromAnim`、动画序列的 `bForceRootLock`，以及移动组件的 `bApplyRootMotion` 相关设置（详见 03 篇 Root Motion 章节）。
...
蒙太奇播放必须在服务器执行并复制（`bReplicates` 的组件/角色调用 `Montage_Play` 且该函数本身是 `BlueprintCallable` 的 RPC 组合），客户端不要各自本地播放不同步的蒙太奇。对演出型蒙太奇，还要考虑用 Group 对齐与"跳过非关键段"（bSyncGroupName 设置）。详见 06 篇。
