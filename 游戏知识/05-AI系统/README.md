...
此外，本分类还收录 UE5 大规模群体 AI 的进阶篇章：**04-Mass 实体框架与群集模拟**（ECS 数据驱动的大规模实体模拟）、**05-StateTree 状态树**（事件驱动、数据绑定的显式决策框架）与 **06-ZoneGraph 与 SmartObjects**（导航走廊与智能对象交互），与前三篇（01~03）互补，共同覆盖从单体 AI 到群体 AI 的全谱系。
...
4. **第四阶段（UE5 大规模 AI，可选）**：学习《04-Mass实体框架与群集模拟》《05-StateTree状态树》《06-ZoneGraph与SmartObjects》，掌握 Mass + StateTree + ZoneGraph/SmartObjects 的群体 AI 组合拳，适合人群、鸟群、车流等大规模场景。
5. **贯穿始终的实践建议**：每篇文档都包含代码/蓝图示例与最佳实践，建议在空白工程中逐个实现；遇到性能问题时，优先使用 UE 自带的 AI 调试工具（`ai.debug.*` 控制台命令、EQS 调试器、NavMesh 可视化）。
