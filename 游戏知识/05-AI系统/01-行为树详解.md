...
| 原生枚举 | `BlackboardKeyType_NativeEnum` | 绑定 UENUM 的枚举 |
...
5. 运行时用 `ai.debug` 系列控制台变量（5.8；旧 `ai.bt` 命令已移除）或调试器观察节点状态（绿色=成功，红色=失败，橙色=运行中）。
...
8. **性能**：使用 `stat AI` 查看开销（5.8 无 `stat BehaviorTree` 组）；大量 AI 时考虑降低 Tick 频率（`BehaviorTreeComponent` 的 `SetComponentTickInterval`）或使用异步 EQS。
