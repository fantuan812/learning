...
> 覆盖：UBT 构建系统、UAT 自动化打包、插件开发与编辑器扩展、资源管理与热更新。
...
这四块内容前后衔接：先会编译，才能谈打包；会打包，才能理解插件与资源的发布形态；理解 Pak/Chunk，才能真正落地热更新方案。
