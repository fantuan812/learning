...
- **开关**：`r.RayTracing.Shadows`，每类光源单独控制（`r.RayTracing.Shadows.Lights.Directional` / `Point` / `Spot` / `Rect`），`r.RayTracing.Shadows.SamplesPerPixel` 默认 1（配合去噪），`MaxBatchSize` 8；
...
| 效果 | 文件 | 核心命令 | 原理与特点 |
| --- | --- | --- | --- |
| RT 反射 | RayTracingReflections | `r.RayTracing.Reflections`（在 DeferredShadingRenderer 中注册）、SamplesPerPixel、MaxBounces | 镜面方向发射反射光线，命中真实场景（含屏幕外物体），粗糙度越高采样越贵；可只对粗糙度低于阈值的像素启用 |
...
- **开启**：`r.PathTracing 1`（配合控制台 `show 1` / 后处理 `Path Tracing` 开关），渲染器替换常规管线；
...
| 反射/AO | `r.RayTracing.Reflections`（含 SamplesPerPixel、MaxBounces、MaxRoughness 等） | 光追反射 |
