...
`USceneComponent` 上有 `bReplicatesAttachment`（挂接关系复制）与 `ReplicateAttachedComponents`（子组件位置复制）等开关，`bReplicateComponentTransform` 控制变换复制。多人游戏中：
...
检查：Actor `bReplicates`；组件 `bReplicatesAttachment` / `bReplicateComponentTransform`；两端挂接是否在相同时机执行（RPC 顺序）；是否依赖了仅本地存在的 Socket。骨骼驱动的持握点建议只复制"挂接关系"，位置交给动画插值。
